...
public class SoundUtility : Utility {
	/*
	public static void SpawnPlayClipAtPoint (string audioClipSetName, Vector3 position, float distance = 30.0f, AudioRolloffMode audioRolloffMode = AudioRolloffMode.Custom) {
		if (!ValidAudioClipSet (audioClipSetName))
			return;
		
		PlayClipAtPoint playClipAtPoint = (Instantiate (Linker.prefabManager.playClipAtPoint, position, Quaternion.identity) as GameObject).GetComponent<PlayClipAtPoint> ();
		
		playClipAtPoint.audio.rolloffMode = audioRolloffMode;
		playClipAtPoint.audio.clip = prefabManager.GetAudioClipSet (audioClipSetName).RandomObject () as AudioClip;
